...
for l in LABELS:
    ...
    wr = real[real["pnl"] > 0]; wl = real[real["pnl"] <= 0]
    wk = keep[keep["pnl"] > 0]
    stats[l] = {
        "real_cnt": len(real), "keep_cnt": len(keep),
        "real_pnl": real["pnl"].sum(), "keep_pnl": keep["pnl"].sum(),
...
def yr_rets(nav_df):
    nav_df = nav_df.copy(); nav_df["y"] = nav_df["week_start"].dt.year
    d = {}
...
def qr_rets(nav_df):
    nav_df = nav_df.copy(); nav_df["q"] = nav_df["week_start"].dt.to_period("Q")
    d = {}
...
def calc_risk(nav_df):
    ...
    wr = np.diff(navs) / navs[:-1]; wr = wr[np.isfinite(wr)]
    ann_ret = np.mean(wr) * 52; ann_vol = np.std(wr) * np.sqrt(52)
    sharpe = ann_ret / ann_vol if ann_vol > 0 else 0
    down = wr[wr < 0]; down_vol = np.std(down) * np.sqrt(52) if len(down) > 0 else 0
    sortino = ann_ret / down_vol if down_vol > 0 else 0
    peaks = np.maximum.accumulate(navs); dd = (navs - peaks) / peaks
    max_dd = dd.min(); calmar = ann_ret / abs(max_dd) if max_dd != 0 else 0
    pos = wr[wr > 0].sum(); neg = abs(wr[wr < 0].sum())
    pf = pos / neg if neg > 0 else 9999
    trough_i = np.argmin(dd); peak_i = np.argmax(navs[:trough_i+1]) if trough_i > 0 else 0
    rec_arr = np.where(navs[trough_i:] >= peaks[trough_i])[0]
    rec_wk = int(trough_i + rec_arr[0] - peak_i) if len(rec_arr) > 0 else None
    # streaks
    win_s = loss_s = cur = 0
    for r in wr:
        if r > 0:   cur = cur + 1 if cur >= 0 else 1
        elif r < 0: cur = cur - 1 if cur <= 0 else -1
        else:       cur = 0
    ...
    return {
        ...
        "avg_up": wr[wr>0].mean()*100, "avg_dn": wr[wr<0].mean()*100,
        "pct_in_dd": (dd < 0).mean()*100,
        "avg_dd": dd[dd<0].mean()*100 if (dd<0).any() else 0,
        "rec_wk": rec_wk, "win_streak": win_s, "loss_streak": loss_s,
    }
...
for l in LABELS:
    ...
    for i, d in enumerate(dd):
        if d < -0.05 and not in_dd:
            ...
            in_dd = False; dd_periods.append((start_i, i, dd[start_i:i].min()))
    if in_dd:
    ...
    for s, e, d in dd_periods:
        print(f"      {pd.Timestamp(dates[s]).strftime('%Y-%m-%d')} -> "
...
for l in LABELS:
    ...
    for l2 in LABELS:
        ...
        only_this = len(syms - s2); only_other = len(s2 - syms); both = len(syms & s2)
...
if "LGBM" in data and "E_70" in data:
    lv_s = data["LGBM"]["summary"]; ev_s = data["E_70"]["summary"]
    lr = risk["LGBM"]; er = risk["E_70"]
    ls = stats["LGBM"]; es = stats["E_70"]
