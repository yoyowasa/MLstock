...
def subperiod_stability():
    ...
    # ---- Return comparison ----
    print(f"{'Year':<6} {'E70(old)':>10} {'feat17':>10} {'V2':>10} "
          f"| {'f17-E70':>9} {'V2-E70':>9}")
    print("-" * 68)
    ...
    for yr in all_years:
        ...
        d_v2  = (r_v2  - r_e70) * 100
        results[yr] = dict(e70=r_e70, f17=r_f17, v2=r_v2,
                           dd_e70=dd_e70, dd_f17=dd_f17, dd_v2=dd_v2)
        print(f"{yr:<6} {r_e70*100:>9.2f}% {r_f17*100:>9.2f}% {r_v2*100:>9.2f}% "
              f"| {d_f17:>+8.2f}pt {d_v2:>+8.2f}pt")
    ...
    print(f"{'feat17':<6} {np.mean(rets_f17)*100:>9.2f}% {np.std(rets_f17)*100:>7.2f}% "
          f"{wins_f17:>4}/{n}")
    print(f"{'V2':<6} {np.mean(rets_v2)*100:>9.2f}% {np.std(rets_v2)*100:>7.2f}% "
          f"{wins_v2:>4}/{n}")
    ...
    if wins_f17 >= n - 1:
        ...
        print(f"[WARN] feat17 beats E70 in only {wins_f17}/{n} full years"
              " -- improvement may be concentrated")
    ...
    if wins_v2 >= n - 1:
        ...
        print(f"[WARN] V2 beats E70 in only {wins_v2}/{n} full years"
              " -- DD benefit may dominate")

    # ---- Drawdown comparison ----
    print()
    print("Max Drawdown by year:")
    print(f"{'Year':<6} {'E70':>9} {'feat17':>9} {'V2':>9} "
          f"| {'f17-E70':>9} {'V2-E70':>9}")
    print("-" * 68)
    for yr in all_years:
        r = results[yr]
        print(f"{yr:<6} {r['dd_e70']*100:>8.2f}% {r['dd_f17']*100:>8.2f}% "
              f"{r['dd_v2']*100:>8.2f}% | "
...
def regime_gate_check():
    ...
    cfgs = {
        "feat17 + gate ON":  replace(cfg,
            selection=replace(cfg.selection, confidence_sizing=False),
            risk=replace(cfg.risk, regime_gate=gate_on)),
        "feat17 + gate OFF": replace(cfg,
            selection=replace(cfg.selection, confidence_sizing=False),
            risk=replace(cfg.risk, regime_gate=gate_off)),
        "V2     + gate ON":  replace(cfg,
            risk=replace(cfg.risk, regime_gate=gate_on)),
        "V2     + gate OFF": replace(cfg,
            risk=replace(cfg.risk, regime_gate=gate_off)),
    }
