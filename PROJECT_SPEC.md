...
> 本ファイルは **weekly パイプライン専用** とし、gap 戦略の仕様は `GAP_PROJECT_SPEC.md` に分離する。
...
2. [目的](#目的)  
3. [スコープ](#スコープ)  
4. [仕様対象とスクリプト境界](#仕様対象とスクリプト境界)  
5. [固定要件（不変）](#固定要件不変)  
6. [追加の固定仕様（このチャットで確定）](#追加の固定仕様このチャットで確定)  
7. [リポジトリと実行環境](#リポジトリと実行環境)  
8. [設定（config）規約](#設定config規約)  
9. [データソースとデータ契約](#データソースとデータ契約)  
10. [パイプライン（初回バックフィル→週次運用）](#パイプライン初回バックフィル週次運用)  
11. [バックテスト仕様（現金・リザーブ・約定）](#バックテスト仕様現金リザーブ約定)  
12. [検証・監査（監査JSON/比較実験）](#検証監査監査json比較実験)  
13. [参考：リスク制御の詳細・比較実験](#参考リスク制御の詳細比較実験)  
14. [未確定事項とバックログ](#未確定事項とバックログ)  
15. [用語・定義](#用語定義)  
...
## 仕様対象とスクリプト境界
本仕様の対象は **weekly パイプラインのみ** とする。gap 戦略のスキャン・日中売買ロジックは本書の対象外であり、`GAP_PROJECT_SPEC.md` を正とする。
...
- `scripts/run_deadband_kpi.py`
- `scripts/run_post_weekly_x.py`
- `scripts/run_backfill_raw.py`
- `scripts/run_build_snapshots.py`
...
### 読み分けルール
- 週次の選定、バックテスト、注文CSV、portfolio 管理は本書を参照する。
- 寄付き gap、intraday entry/exit、scan-only、replay-log は `GAP_PROJECT_SPEC.md` を参照する。
